...
uniform float mouse_x_offset; // 0.0
varying vec2 v_texCoords;

uniform float edge_thres; // 0.2;
uniform float edge_thres2; // 10.0;
...
{
  float dxtex = 1.0 /float(textureSize(u_texture,0)) ;
  float dytex = 1.0 /float(textureSize(u_texture,0));
  float pix[9];
  int k = -1;
  float delta;
...
void main()
{
  ...
  {
    vec3 colorOrg = texture2D(u_texture, uv).rgb;
